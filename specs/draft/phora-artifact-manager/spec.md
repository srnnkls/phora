...
```toml
# Generated by phora - do not edit
version = 1
...
target = "neovim"
artifacts = [
  { name = "nvim", source = "dotfiles" },
]

[[projections]]
target = "vscode"
artifacts = [
  { name = "settings", source = "dotfiles" },
  { name = "keybindings", source = "dotfiles" },
  { name = "snippets", source = "company-configs" },
]
ejected = []

[[projections]]
target = "cupcake-policies"
artifacts = [
  { name = "python", source = "loqui" },
  { name = "go", source = "loqui" },
]
```
...
pub struct Lock {
    ...
    pub projections: Vec<LockedProjection>,
}
...
impl Lock {
    ...
    pub fn find_projection(&self, target: &str) -> Option<&LockedProjection> {
...
pub fn check_artifact_state(
    ...
    if ejected.contains(&artifact_name.to_string()) {
...
pub fn sync(config: &Config, lock: Option<Lock>, force: bool, interactive: bool) -> Result<Lock, Error> {
    ...
    for (name, source) in &config.sources {
        let locked = lock.as_ref().and_then(|l| l.find_source(name));
        ...
        new_lock.sources.push(LockedSource {
            name: name.clone(),
    ...
    for (target_name, target) in &config.targets {
        ...
        let sources = target.resolve_sources(&config.sources);

        let strategy = detect_strategy(&phora_dir().join("cache"), &target_path);

        let mut ejected = lock
            .as_ref()
            .and_then(|l| l.find_projection(target_name))
            .map(|p| p.ejected.clone())
            .unwrap_or_default();
        ...
        for source_name in sources {
            let source = &config.sources[source_name];
            let locked = new_lock.find_source(source_name).unwrap();
            ...
            for artifact_name in discovered {
                ...
                match state {
                    ...
                    | ArtifactState::Foreign => {
                        ...
                        if let Some(pos) = ejected.iter().position(|a| a == &artifact_name) {
        ...
        new_lock.projections.push(LockedProjection {
            target: target_name.clone(),
            artifacts,
            ejected,
        });
...
pub fn eject(
    ...
    let projection = new_lock
        .projections
        .iter_mut()
        .find(|p| p.target == target)
        .ok_or(Error::TargetNotFound { target: target.to_string() })?;

    // Validate artifact was managed
    let was_managed = projection
        .artifacts
        .iter()
        .any(|a| a.name == artifact && a.source == source);
    ...
    // Remove from artifacts list (match full identity)
    projection.artifacts.retain(|a| !(a.name == artifact && a.source == source));

    // Add to ejected
    if !projection.ejected.contains(&artifact.to_string()) {
        projection.ejected.push(artifact.to_string());
    }

    // Remove registry record (authoritative state)
    let key = ArtifactKey {
        target: target.to_string(),
        source: source.to_string(),
        artifact: artifact.to_string(),
    ...
    let _ = artifact_path; // kept for potential UX output
...
* Then artifact marked ejected in lock, registry record deleted, files untouched
...
* Given artifact previously ejected and later deleted by user
  ...
  * Then it is removed from the lock ejected list (clearing ejected on restore)
