...
fn dep_repo(dir: &Path, inner_name: &str, inner_url: &str) {
    ...
    let manifest = format!(
        "version = 1\n\n[sources.{inner_name}]\ngit = \"{inner_url}\"\ntransitive = true\n\n\
         [targets.t]\npath = \"sub\"\nsources.{inner_name} = {{ collapse = false }}\n",
    );
...
#[test]
fn bound_transitive_source_deploys_own_variants_and_dependencies_then_replays_offline() {
    let fixture = build_fixture();
    let guides = fixture.cwd.path().join("guides");
    let package = fixture.cwd.path().join("package");
    for directory in [&guides, &package] {
        std::fs::create_dir(directory).unwrap();
        git(directory, &["init", "-b", "claude", "."]);
        git(directory, &["config", "user.email", "test@example.com"]);
        git(directory, &["config", "user.name", "Test"]);
    }
    write(&guides.join("README.md"), b"pinned Loqui guide\n");
    git(&guides, &["add", "."]);
    git(&guides, &["commit", "-m", "guides"]);
    let remote = "https://bound-transitive.invalid/loqui.git";
    write(
        &fixture.home_path.join(".gitconfig"),
        format!("[url {guides:?}]\n    insteadOf = {remote}\n").as_bytes(),
    );
    for harness in ["claude", "codex"] {
        if harness == "codex" {
            git(&package, &["switch", "-c", "codex"]);
        }
        write(&package.join("skills/code/SKILL.md"), harness.as_bytes());
        write(
            &package.join("phora.toml"),
            format!(
                "[sources.loqui]\ngit = {remote:?}\nbranch = \"claude\"\n\
             [targets.loqui]\npath = \"skills/loqui/reference/{harness}\"\nsources = [\"loqui\"]\n"
            )
            .as_bytes(),
        );
        git(&package, &["add", "."]);
        git(&package, &["commit", "-m", harness]);
    }
    write(&fixture.cwd.path().join("phora.toml"), format!(
        "[sources.tropos]\npath = {package:?}\ntransitive = true\ninclude = [\"skills/**\"]\n\
         [targets.claude]\npath = \"deployed/.claude\"\nsources.tropos = {{ branch = \"claude\", collapse = false }}\n\
         [targets.codex]\npath = \"deployed/.codex\"\nsources.tropos = {{ branch = \"codex\", collapse = false }}\n"
    ).as_bytes());
    let unpinned = run(&fixture, &["sync", "--frozen"]);
    assert!(!unpinned.status.success());
    assert!(!fixture.cwd.path().join("deployed").exists());
    let first = run(&fixture, &["sync", "--no-hooks"]);
    assert!(
    ...
    for harness in ["claude", "codex"] {
        let target = fixture.cwd.path().join(format!("deployed/.{harness}"));
        assert_eq!(
            std::fs::read(target.join("skills/code/SKILL.md")).unwrap(),
            harness.as_bytes()
        );
        assert_eq!(
            std::fs::read(target.join(format!("skills/loqui/reference/{harness}/README.md")))
                .unwrap(),
            b"pinned Loqui guide\n"
        );
    }
    write(&package.join("skills/code/SKILL.md"), b"codex rebuilt");
    let updated_manifest = std::fs::read_to_string(package.join("phora.toml"))
        .unwrap()
        .replace("reference/codex", "reference/codex-rebuilt");
    write(&package.join("phora.toml"), updated_manifest.as_bytes());
    git(&package, &["add", "."]);
    git(&package, &["commit", "-m", "rebuild codex"]);
    let locked = run(&fixture, &["sync", "--no-hooks"]);
    assert!(
    ...
    assert_eq!(
        std::fs::read(
            fixture
                .cwd
                .path()
                .join("deployed/.codex/skills/code/SKILL.md")
        )
        .unwrap(),
        b"codex"
    );
    assert!(
        fixture
            .cwd
            .path()
            .join("deployed/.codex/skills/loqui/reference/codex/README.md")
            .is_file()
    );
    assert!(
        !fixture
            .cwd
            .path()
            .join("deployed/.codex/skills/loqui/reference/codex-rebuilt")
            .exists()
    );
    let updated = run(&fixture, &["update", "tropos", "--fast-forward"]);
    assert!(
        updated.status.success(),
        "{}",
        String::from_utf8_lossy(&updated.stderr)
    );
    assert!(!String::from_utf8_lossy(&updated.stderr).contains("foreign content"));
    assert_eq!(
        std::fs::read(
            fixture
                .cwd
    ...
    assert!(
        fixture
            .cwd
            .path()
            .join("deployed/.codex/skills/loqui/reference/codex-rebuilt/README.md")
            .is_file()
    );
    let lock_path = fixture.cwd.path().join("phora.lock");
    let before = std::fs::read(&lock_path).unwrap();
    let lock: toml::Value = toml::from_str(std::str::from_utf8(&before).unwrap()).unwrap();
    let roots = lock["sources"]
        .as_array()
        .unwrap()
        .iter()
        .filter(|entry| entry["name"].as_str() == Some("tropos"))
        .count();
    assert_eq!(roots, 2, "each binding ref must retain its own root pin");
    std::fs::rename(&package, package.with_extension("offline")).unwrap();
    std::fs::rename(&guides, guides.with_extension("offline")).unwrap();
    let frozen = run(&fixture, &["sync", "--frozen", "--no-hooks"]);
    assert!(
        frozen.status.success(),
        "{}",
    ...
    let verified = run(&fixture, &["verify"]);
    assert!(
...
#[test]
fn bound_transitive_source_still_rejects_a_dependency_owned_local_remote() {
    let fixture = build_fixture();
    let package = fixture.cwd.path().join("package");
    std::fs::create_dir(&package).unwrap();
    dep_repo(&package, "escape", "/etc");
    write(
    ...
    let output = run(&fixture, &["sync"]);
    assert!(!output.status.success());
    assert!(String::from_utf8_lossy(&output.stderr).contains(TRANSITIVE_ESCAPE_DIAGNOSTIC));
    assert!(!fixture.cwd.path().join("deployed").exists());
    assert!(!fixture.cwd.path().join("phora.lock").exists());
}
